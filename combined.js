...
const k_collector = function() {
  ...
  this.toCode = function(indexOffset){
    ...
    codeString += "    real(kind_phys),           intent(in)  :: TEMP ! temperature\n"
    codeString += "    real(kind_phys),           intent(out) :: k_rate_constants(:) ! rate constant for the each reaction\n"
...
const j_rate_map_collection = function(){
  ...
  this.toCode = function(indexOffset){
    ...
    codeString += "    real(kind_phys),           intent(in)  :: tuv_rates(:) ! /sec \n"
    codeString += "    real(kind_phys),           intent(out) :: j_rateConst(:) ! /sec \n"
    for(let i=0; i< this.mapping.length; i++){
...
app.post('/constructJacobian', function(req, res) {
  ...
  rate_constant_module += "use ccpp_kinds, only: kind_phys \n\n"
